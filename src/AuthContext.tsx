      ...
      if (firebaseUser) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            // Create new user (default to student for now)
            ...
            const newUser = {
              ...
              tenantId: isSuperAdminEmail ? 'SYSTEM' : 'tenant_demo_1',
              tenantName: isSuperAdminEmail ? 'Plateforme SaaS' : 'Université Démo UCCM',
  ...
  const signInAsDemo = async (role: UserData['role']) => {
    try {
      ...
      if (error.code === 'auth/operation-not-allowed') {
        alert("L'authentification anonyme n'est pas activée. Veuillez l'activer dans la console Firebase (Authentication > Sign-in method > Anonymous).");
      } else if (error.code === 'auth/admin-restricted-operation') {
        alert("Erreur de permission Firebase. Pour utiliser le mode démo, vous devez :\n\n1. Activer l'authentification Anonyme (Authentication > Sign-in method)\n2. Activer la création de compte (Authentication > Settings > User Actions > Cochez 'Enable create (sign-up)')");
      } else {
        alert("Erreur lors de la connexion de démonstration : " + error.message);
      }
    }
  };
