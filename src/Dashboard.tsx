...
const ShortcutButton = ({ icon, title, subtitle, colorClass }: any) => {
  ...
  return (
    <button className="flex items-center gap-4 p-5 bg-white rounded-xl border border-slate-200 hover:border-blue-500 hover:shadow-lg hover:-translate-y-1 transition-all group w-full text-left">
...
export default function Dashboard() {
  ...
  const navigation = [
    { id: 'overview', name: "Vue d'ensemble", icon: LayoutDashboard, roles: ['admin', 'cashier', 'chef', 'super_admin'] },
    { id: 'institutions', name: 'Institutions SaaS', icon: Building2, roles: ['super_admin'] },
    { id: 'users', name: 'Utilisateurs', icon: Users, roles: ['admin', 'super_admin'] },
    { id: 'documents', name: 'Documents & TFC', icon: FileText, roles: ['admin', 'chef', 'super_admin'] },
    { id: 'payments', name: 'Paiements', icon: CircleDollarSign, roles: ['admin', 'cashier', 'super_admin'] },
    { id: 'ai', name: 'Outils IA', icon: Sparkles, roles: ['admin', 'super_admin', 'chef', 'cashier'] },
    { id: 'settings', name: 'Paramètres', icon: Settings, roles: ['admin', 'super_admin'] },
  ];
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans antialiased overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between h-16 px-6 bg-slate-950/50 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-extrabold text-white tracking-tight">UCCM</span>
          </div>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/30">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-300 font-bold uppercase shrink-0">
              {user?.name.charAt(0)}
            </div>
          ...
          <button 
            onClick={signOut}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-lg text-sm font-medium transition-colors"
          >
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
            ...
            <div className="space-y-6 max-w-7xl mx-auto">
              ...
              {user?.role === 'admin' && (
                <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <ShortcutButton icon={<UserPlus className="h-5 w-5" />} title="Nouvel Utilisateur" subtitle="Créer un compte" colorClass="blue" />
                  <ShortcutButton icon={<CircleDollarSign className="h-5 w-5" />} title="Encaissement" subtitle="Frais & Dépôts" colorClass="emerald" />
                  <ShortcutButton icon={<FileText className="h-5 w-5" />} title="Rapports" subtitle="Journalier & Hebdo" colorClass="amber" />
                  <ShortcutButton icon={<Settings className="h-5 w-5" />} title="Système" subtitle="Paramètres" colorClass="slate" />
                </section>
              )}

              {user?.role === 'cashier' && (
                <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <ShortcutButton icon={<CircleDollarSign className="h-5 w-5" />} title="Nouveau Paiement" subtitle="Enregistrer" colorClass="emerald" />
                  <ShortcutButton icon={<FileText className="h-5 w-5" />} title="Reçus" subtitle="Historique" colorClass="blue" />
                </section>
              )}

              {user?.role === 'chef' && (
                <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <ShortcutButton icon={<FileText className="h-5 w-5" />} title="Nouveau Document" subtitle="Ajouter" colorClass="blue" />
                  <ShortcutButton icon={<Users className="h-5 w-5" />} title="Étudiants" subtitle="Liste" colorClass="emerald" />
                </section>
              )}
            </div>
          )}
          ...
          {activeTab === 'notifications' && (
            <div className="max-w-7xl mx-auto space-y-6">
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <h2 className="text-2xl font-bold text-slate-800 mb-2">Toutes les notifications</h2>
                <p className="text-slate-500">Cette vue détaillée sera implémentée prochainement.</p>
              </div>
            </div>
          )}
          ...
          {/* PLACEHOLDER TABS */}
          {activeTab !== 'overview' && activeTab !== 'users' && activeTab !== 'documents' && activeTab !== 'payments' && activeTab !== 'institutions' && activeTab !== 'ai' && (
